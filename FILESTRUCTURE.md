...
* C++ source files:
  * `include/rml/rml.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/rml.cpp` is the main file that implements this library.
  * `app/rml_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/rml_t.cpp` contains the unit tests for the library.
  ...
  * The `python/rml` directory contains a Python
    package for the project. It contains a compiled Python module `_rml`
    that CMake generates from the Pybind11 source file `python/rml/_rml.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
